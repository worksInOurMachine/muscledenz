...
interface VelocityTextProps {
  children: React.ReactNode; // Accepts HTML elements
  baseVelocity: number;
...
interface SlidingTextProps {
  scrollContainerRef?: React.RefObject<HTMLElement>;
  texts: React.ReactNode[]; // Now it supports JSX elements
  velocity?: number;
  className?: string;
  damping?: number;
  stiffness?: number;
...
export const SlidingText: React.FC<SlidingTextProps> = ({
  ...
  function VelocityText({
    ...
    const SlidingText = useVelocity(scrollY);
    const smoothVelocity = useSpring(SlidingText, {
      damping: damping ?? 50,
